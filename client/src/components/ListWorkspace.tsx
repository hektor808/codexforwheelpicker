...
export function ListWorkspace({ list, onListChange, onListDelete }: ListWorkspaceProps) {
  ...
  const hasChanges = useMemo(() => {
    ...
    const normalized = items.filter((item) => item.label.trim().length > 0);
    if (normalized.length !== list.items.length) return true;
    if (list.name !== name.trim()) return true;
    if ((list.description ?? '') !== description.trim()) return true;
    return normalized.some((item, index) => {
      const original = list.items[index];
      return original.label !== item.label.trim() || original.weight !== item.weight;
    });
  }, [items, name, description, list]);
  ...
  async function handleSave() {
    ...
    const payload = {
      ...
      items: items.filter((item) => item.label.trim().length > 0).map((item) => ({
        id: item.id,
  ...
  return (
    <div className="flex h-full flex-col gap-6 overflow-y-auto p-10">
                ...
                <input
                  ...
                  onChange={(event) =>
                    handleItemChange(index, (current) => {
                      ...
                      const weight = Number.isNaN(parsed) ? 1 : Math.max(1, parsed);
                      return {
                        ...current,
                        weight,
                      };
      ...
      <div className="grid gap-6 lg:grid-cols-[minmax(280px,360px)_1fr]">
        <Wheel key={lastSpinItemId ?? list.id} items={list.items} lastSpinItemId={lastSpinItemId} />
